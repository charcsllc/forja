...
export default function WorkspacePage() {
  ...
  /* The workspace: warm base under everything, white cards for the panels, hairlines. */
  const pageBg = "var(--surface-1)";
  const cardBg = "var(--surface-2)";

  if (loading) return <div className="h-screen flex flex-col items-center justify-center gap-3 text-ink" style={{ background: pageBg }}><Loader2 className="size-7 animate-spin text-ink-4" /><p className="text-sm text-ink-3">{"Loading…"}</p></div>;
  if (!project) return <div className="h-screen flex flex-col items-center justify-center gap-4 text-ink" style={{ background: pageBg }}><p className="text-ink-3">Project not found</p><Link href="/"><Button variant="outline">{"Back"}</Button></Link></div>;

  // Popup menu content (shared between desktop and mobile)
  const popupMenu = menuOpen && (
    <div data-popup-menu className="absolute top-full left-0 z-[60] mt-1.5 w-60 overflow-hidden rounded-2xl bg-popover p-1 shadow-floating">
      <div className="border-b border-hairline px-3 py-2">
        <p className="truncate text-xs text-ink-3">{projectId}</p>
      </div>
      <div className="py-1">
        <button onClick={() => { setMenuOpen(false); router.push("/"); }}
          className="w-full flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors text-ink-2">
          <FolderOpen className="size-4 text-ink-4" /> {"My projects"}
        </button>
        <button onClick={() => { setMenuOpen(false); router.push("/"); }}
          className="w-full flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors text-ink-2">
          <Plus className="size-4 text-ink-4" /> {"New project"}
        </button>
      </div>
      ...
      <div className="sm:hidden border-t border-hairline py-1" >
        {TABS.map((tab) => (
          <button key={tab.id} onClick={() => { setActiveTab(tab.id); setMobileTab("panel"); setMenuOpen(false); }}
            className={`w-full flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors ${activeTab === tab.id ? "text-ink font-medium" : "text-ink-2"}`}>
            <tab.icon className="size-4 text-ink-4" /> {tab.label}
          </button>
        ))}
      </div>
          ...
          return (
            <button key={entry.id} onClick={() => { openEntry(entry.id); setMenuOpen(false); }}
              ...
              <span className="relative flex items-center">
                <entry.icon className="size-4 text-ink-4" />
                {connected && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-surface-2" />}
              </span>
              <span className="flex-1 text-left">{entry.label}</span>
              {connected && <span className="text-[10px] font-medium text-positive">{"Connected"}</span>}
            </button>
          );
        })}
        <button onClick={() => { setMenuOpen(false); void openClone(); }}
          className="w-full flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors text-ink-2">
          <Copy className="size-4 text-ink-4" /> {"Duplicate project"}
        </button>
      </div>
      <div className="border-t border-hairline py-1" >
        {/* Publish on mobile */}
        <button onClick={() => { handleDeploy(); setMenuOpen(false); }} disabled={deploying || isBuilding}
          className="w-full sm:hidden flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors text-ink-2">
          <Rocket className="size-4 text-ink-4" /> {deploying ? "Deploying" : "Publish"}
        </button>
      </div>
      <div className="border-t border-hairline py-1" >
        <button onClick={() => { handleRestartServer(); setMenuOpen(false); }}
          className="w-full flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-muted transition-colors text-ink-2">
          <Server className="size-4 text-ink-4" /> {"Restart Agent Server"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden text-ink" style={{ background: pageBg }} onClickCapture={markWorkspaceTouched}>
      ...
      <div className="hidden sm:flex flex-col h-full">
        ...
        <header data-workspace-header className="flex items-stretch shrink-0 z-10" style={{ height: 48 }}>
          ...
          <div className="flex items-center gap-1.5 px-3 shrink-0" style={{ width: typeof leftHeaderWidth === "number" ? leftHeaderWidth : undefined }}>
            <Link href="/" title={"Back"} className="flex size-8 items-center justify-center rounded-full text-ink-3 hover:text-ink hover:bg-muted transition-colors shrink-0">
              <ArrowLeft className="w-4 h-4" />
            </Link>
            <div className="relative flex-1 min-w-0" ref={menuRef}>
              <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-1.5 max-w-full rounded-lg px-1.5 py-1 hover:bg-muted transition-colors">
                <LogoMark size={18} />
                <span className="truncate text-sm font-medium text-ink">{project.label || projectId}</span>
          ...
          <div className="flex items-center flex-1 min-w-0 gap-1.5 px-3">
            <div className="bb-hairline flex h-8 shrink-0 items-center gap-0.5 rounded-full bg-surface-2 p-0.5">
              {TABS.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} aria-pressed={activeTab === tab.id}
                  className={`flex h-7 items-center gap-1.5 rounded-full px-3 text-sm transition-colors ${activeTab === tab.id ? "bg-muted text-ink" : "text-ink-3 hover:text-ink"}`}>
                  <tab.icon className="size-3.5" /><span className="hidden lg:inline">{tab.label}</span>
                </button>
              ))}
            </div>
            <div className="flex-1 flex items-center justify-center min-w-0">
              <div className="bb-hairline flex h-8 w-[340px] items-center gap-1 rounded-full bg-surface-2 px-1.5">
                ...
                */}
                ...
                />
                <button className="p-1 rounded text-ink-3 hover:text-ink shrink-0" onClick={() => { fetchProject(); setPreviewKey((k) => k + 1); }}><RefreshCw className="w-3.5 h-3.5" /></button>
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className={`flex flex-col shrink-0 transition-all ${chatCollapsed ? "w-0 overflow-hidden" : ""}`} style={chatCollapsed ? {} : { width: chatWidth, background: pageBg }}>
            <ChatPanel
          ...
          {!chatCollapsed && (
            <div className="flex w-3 shrink-0 cursor-col-resize items-center justify-center transition-colors hover:bg-muted/40" onMouseDown={handleResizeStart}>
              <div className="h-8 w-0.5 rounded-full bg-hairline" />
            </div>
          )}
          <div className="flex-1 flex flex-col min-w-0">
            ...
            */}
            ...
            <div className="bb-hairline m-2 flex-1 overflow-hidden rounded-2xl sm:m-3 sm:ml-0" style={{ background: cardBg }}>
          ...
          */}
          {visualEditorOpen && (
            <div className="w-[19rem] shrink-0 overflow-hidden border-l border-hairline bg-surface-2 xl:w-[21rem]">
              <VisualEditorPanel
                projectId={projectId}
      ...
      <div className="flex sm:hidden flex-col h-full">
        ...
        <header data-workspace-header className="flex items-center gap-1 px-2 shrink-0 z-10" style={{ height: 44 }}>
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-1.5 px-1.5 py-1 rounded-lg hover:bg-muted transition-colors">
              <LogoMark size={18} />
              <span className="max-w-[160px] truncate text-sm font-medium text-ink">{project.label || projectId}</span>
              <ChevronDown className="size-3 shrink-0 text-ink-4" />
            </button>
            {popupMenu}
          </div>
        ...
        */}
        ...
        <div className="shrink-0 flex items-center justify-center py-2 px-4" style={{ background: pageBg }}>
          <div className="flex w-full max-w-xs rounded-full bg-muted p-1">
            <button onClick={() => setMobileTab("panel")} className={`w-1/2 rounded-full py-2 text-center text-sm font-medium transition-colors ${mobileTab === "panel" ? "bb-hairline bg-surface-2 text-ink" : "text-ink-3"}`}>{"Preview"}</button>
            <button onClick={() => setMobileTab("chat")} className={`w-1/2 rounded-full py-2 text-center text-sm font-medium transition-colors ${mobileTab === "chat" ? "bb-hairline bg-surface-2 text-ink" : "text-ink-3"}`}>{"Chat"}</button>
          </div>
        </div>
      </div>
