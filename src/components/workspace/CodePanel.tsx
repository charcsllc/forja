...
// ── Cache constants ──────────────────────────────────────────────────────────
const CACHE_PREFIX = `${STORAGE_PREFIX}-code-`;
...
function iconForFile(path: string) {
  ...
  switch (ext) {
    ...
    case "md": case "mdx": case "txt":
      return <FileText className="w-3.5 h-3.5 text-ink-4 shrink-0" />;
    ...
    default:
      return <FileIcon className="w-3.5 h-3.5 text-ink-4 shrink-0" />;
  }
}
    ...
    return (
      <div>
        <button
          ...
          className="w-full flex items-center gap-1.5 pr-2 py-1 rounded-md text-[13px] text-ink-2 dark:text-ink-4 hover:bg-muted dark:hover:bg-primary/70 transition-colors group"
        >
          <ChevronRight
            className={`w-3.5 h-3.5 text-ink-4 shrink-0 transition-transform ${isOpen ? "rotate-90" : ""}`}
          />
          {isOpen ? (
    ...
    <button
      ...
      className={`w-full flex items-center gap-1.5 pr-2 py-1 rounded-md text-[13px] transition-colors ${
        isSelected
          ...
          : "text-ink-2 dark:text-ink-4 hover:bg-muted dark:hover:bg-primary/70"
      }`}
    >
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-ink-4">{"Loading source code..."}</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-ink-2 dark:text-ink-4 mb-1">{"Failed to load source code"}</p>
          <p className="text-xs text-ink-4 max-w-sm break-words">{String(error)}</p>
        </div>
        <button
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-3 py-2 border-b border-hairline  flex items-center gap-2 shrink-0">
        ...
        <span className="text-sm font-medium text-ink-2 dark:text-ink-4">{"Code Explorer"}</span>
        <span className="text-[11px] text-ink-4">· {filesCount} {"files"}</span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={() => fetchCode(true)}
            className="flex items-center gap-1.5 h-7 px-2.5 rounded-lg text-xs font-medium text-ink-3 dark:text-ink-4 border border-hairline  hover:bg-muted dark:hover:bg-primary transition-colors"
            title={"Refresh"}
          >
      ...
      <div className="flex-1 flex min-h-0">
        {/* Tree sidebar */}
        <div className="w-64 shrink-0 border-r border-hairline  flex flex-col bg-muted/40 dark:bg-primary/40">
          {/* Search by file name or content */}
          <div className="p-2 border-b border-hairline  shrink-0">
            <div className="relative">
              <Search className="w-3.5 h-3.5 text-ink-4 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={"Search by file name or content..."}
                className="w-full h-8 pl-8 pr-7 rounded-lg text-xs bg-white dark:bg-primary border border-hairline  text-ink-2 dark:text-ink-4 placeholder:text-ink-4 outline-none focus:ring-2 focus:ring-violet-200 dark:focus:ring-violet-900/40 transition-shadow"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded text-ink-4 hover:text-ink-2 dark:hover:text-ink-4"
                  title="Clear"
                >
          ...
          <div className="flex-1 overflow-y-auto py-1.5 px-1.5">
            {tree.children.length === 0 ? (
              <p className="text-xs text-ink-4 p-3">{query ? "No files match your search" : "Failed to load source code"}</p>
            ) : (
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          {!selected ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 px-6">
              <div className="w-14 h-14 rounded-2xl bg-muted dark:bg-primary border border-dashed border-hairline  flex items-center justify-center">
                <FileCode2 className="w-6 h-6 text-ink-4 dark:text-ink-2" />
              </div>
              <p className="text-sm text-ink-4">{"Select a file to view its contents"}</p>
            </div>
          ) : (
            <>
              {/* Breadcrumb + the save control */}
              <div className="px-3 py-1.5 border-b border-hairline  flex items-center gap-2 shrink-0">
                <code className="text-xs font-mono text-ink-2 dark:text-ink-4 truncate">{selected}</code>
                ...
                <span className="text-[10px] text-ink-4 shrink-0">{humanSize(selectedSize)}</span>
...
function PlaceholderBinary({ onDownload }: { onDownload: () => void }) {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center px-6">
      <div className="w-16 h-16 rounded-2xl bg-muted dark:bg-primary border border-hairline  flex items-center justify-center">
        <FileWarning className="w-7 h-7 text-ink-4 dark:text-ink-2" />
      </div>
      <p className="text-sm text-ink-3 dark:text-ink-4 max-w-xs">{"Preview not available for this file type"}</p>
      <button
        onClick={onDownload}
        className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium transition-colors"
      >
